...
| 自选股管理 | 可视化管理页 | Web 新增股票管理页，展示股票名称、代码、添加日期、添加后涨幅、缓存更新时间；支持弹窗批量输入代码（逗号/换行分隔）、后台非阻塞添加、运行中取消后台添加、刷新后继续恢复进度、后端统一校验并将成功项同步进自选股列表、失败时仅展示失败代码、新增时优先只拉取股票名称和当前价等轻量信息以提升速度、增删股票、手动刷新和一键调用原有分析流程，并与 `STOCK_LIST` 自动同步 |
| 全局日志 | 后端持久化日志页 | Web 新增全局日志页面，统一展示最近操作记录；股票批量新增的逐条成功/失败结果与汇总结果会持久化到后端数据库，并可按分类/状态筛选查看 |
...
> Web 端新增“股票管理”页面，自选股新增/删除会同步回写 `.env` 中的 `STOCK_LIST`；如果仍然通过设置页或直接编辑 `.env` 修改 `STOCK_LIST`，管理页会在读取时自动同步这些变更。页面中的“添加后涨幅”默认走缓存，仅在手动刷新时强制更新，平时会在对应市场收盘后按最新日线自动更新一次。批量新增改为后台任务执行，由后端统一校验；成功项会在任务完成后同步进入自选股列表，失败时页面仅展示失败代码，刷新后会继续恢复任务进度。运行中的后台添加支持取消，点击取消后页面会立即进入已取消状态；当前正在处理的股票会在后台自然收尾，但不会继续执行剩余任务。新增时优先请求名称和当前价等基础信息，不再为单次新增额外拉取日线补齐，减少等待时间。

> Web 端新增“全局日志”页面，日志数据持久化保存在后端数据库中；当前已接入自选股批量新增日志，逐条添加结果和整批汇总结果都可在日志页查看。
